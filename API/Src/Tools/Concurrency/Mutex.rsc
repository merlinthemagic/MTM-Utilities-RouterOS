...
:set ($s->"lock") do={
	...
	:if ([:typeof $1] = "num") do={
		:if ($1 < 2) do={
			:error ($cPath.": Input lock duration for lock name '".$0."' too short, must be at least 2s");
		} else={
			:set hold $1;
		}
	} else={
	...
	:if ([:typeof $2] = "num") do={
		:set wait $2;
		:if ($wait < 0) do={
			:error ($cPath.": Input lock wait for lock name '".$0."' too short, must be at least 0s");
		}
		
	} else={
		:if ([:typeof $2] != nil) do={
			:error ($cPath.": Input lock wait time invalid type '".[:typeof $0]."'");
		}
	}
	...
	:while ($isDone = false) do={
		:set lock ($MtmUtilMutex->$0);
		:if ([:typeof $lock] = "nothing") do={
			:set ($MtmUtilMutex->$0) {expire=($cTime + $hold);key=$key};
			:set isDone true;
		} else={
