:local cPath "MTM/Utilities/Tools/Time/Routeros.rsc";

:local s [:toarray ""];

:set ($s->"getMiliFromFormat") do={

	:local cPath "MTM/Utilities/Tools/Time/ROS.rsc/getMiliFromFormat";
	:if ([:typeof $0] != "str") do={
...
:set ($s->"getGmtOffset") do={
	...
	:local cPath "MTM/Utilities/Tools/Time/ROS.rsc/getGmtOffset";
	...
	:if (($mVal ~ "^-[0-9]{0,5}\$") = false && ($mVal ~ "^[0-9]{0,5}\$") = false) do={
		...
		:global MtmUtilTime;
		:local self ($MtmUtilTime->"routeros");
		:if ($mIsNeg = false) do={
...
:set ($s->"getSecondsFromFormat") do={
	...
	:local cPath "MTM/Utilities/Tools/Time/ROS.rsc/getSecondsFromFormat";
	:if ([:typeof $0] != "str") do={
		:error ($cPath.": Input time has invalid type '".[:typeof $0]."'");
	}
	:if ([:typeof $1] != "str") do={
...
:set ($s->"getCurrentHour") do={
	...
	:local cPath "MTM/Utilities/Tools/Time/ROS.rsc/getCurrentHour";
	:return [:tonum [:pick [/system/clock/get time] 0 2]];
}
:global MtmUtilTime;
:set ($MtmUtilTime->"routeros") $s;
