:local cPath "MTM/Utilities/Tools/Time/Epoch.rsc";

:local s [:toarray ""];

:set ($s->"getCurrent") do={

	:local cPath "MTM/Utilities/Tools/Time/Epoch.rsc/getCurrent";
	:global MtmUtils;

	:local format [($MtmUtils->"getAvp") "mtm.utils.ros.time.format" (false)];
	:if ([:typeof $format] != "str") do={
		:local mVal [($MtmUtils->"get") "getTools()->getPackages()->getRouteros()->getMinor()"];
		:if ($mVal > 9) do={
			:set format "Y-m-d H:i:s";
		} else={
			:set format "m/d/Y h:i:s";
		}
		:set mVal [($MtmUtils->"setAvp") "mtm.utils.ros.time.format" $format];
	}
	:global MtmUtilTime;
	:local self ($MtmUtilTime->"epoch");
	:return [($self->"getFromFormat") [:tostr ([/system/clock/get date]." ".[/system/clock/get time])] $format];
}

:set ($s->"getFromFormat") do={

	:local cPath "MTM/Utilities/Tools/Time/Epoch.rsc/getFromFormat";
	:if ([:typeof $0] != "str") do={
	...
	:global MtmUtils;
	:set d ($d - ([($MtmUtils->"get") "getTools()->getTime()->getRouteros()->getGmtOffset()"]));
	:if ($d > 946684799) do={
